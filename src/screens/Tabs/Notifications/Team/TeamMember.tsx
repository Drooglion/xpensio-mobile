...
class TeamMember extends PureComponent {
  ...
  renderVirtualCardRequest = notifications => {
    const requests = notifications.filter(
      item => item.type === 'request' && item.cardType === 'virtual',
    );
    return requests.length > 0 ? (
      <View>
        <Text style={styles.bodyText}>
          {R.strings.memberRequestedVirtualCard}
        </Text>
        <View style={styles.horizontalActions}>
          <Button transparent style={styles.approveBtn}>
            <Text uppercase style={styles.approveText}>
              {R.strings.approve}
            </Text>
          </Button>
          <Button transparent style={styles.denyBtn}>
            <Text uppercase style={styles.denyText}>
              {R.strings.deny}
            </Text>
          </Button>
        </View>
  ...
  renderPlasticCardRequest = notifications => {
    const requests = notifications.filter(
      item => item.type === 'request' && item.cardType === 'plastic',
    );
    return requests.length > 0 ? (
      <View>
        <Text style={styles.bodyText}>
          {R.strings.memberRequestedPlasticCard}
        </Text>
        <View style={styles.horizontalActions}>
          <Button transparent style={styles.approveBtn}>
            <Text uppercase style={styles.approveText}>
  ...
  render() {
    ...
    const { memberId, teamId, firstName, lastName } = member;
      ...
      <StyleProvider style={getTheme(theme)}>
        <Container>
          ...
          <Content contentContainerStyle={styles.content}>
            <Query query={TEAMS.TEAM_MEMBER} variables={{ teamId, memberId }}>
              {({ error, loading, data }) => {
                if (error) {
                  HelperUtils.bugsnag.notify(error);
                  return null;
                ...
                const virtual = payload.cards.filter(
                  item => item.type === 'virtual',
                );
                const plastic = payload.cards.filter(
                  item => item.type === 'plastic',
                );
                return (
                    ...
                    <View style={styles.section}>
                      ...
                      </Text>
                      {
                        ...
                        virtual.length > 0 ? (
                          <MemberCardsList
                            data={virtual}
                            onItemClick={this.onCardActionClick}
                          />
                        ) : (
                          <Button solid primary style={styles.issueCardBtn}>
