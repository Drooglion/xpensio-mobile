...
const PaymentsDetails = ({
  ...
  let tab = {};
  const { companyConfiguration: { currency } } = companyConfiguration;
  ...
  const handleRejectPayment = async () => {
    ...
    try {
      ...
      const { data: { payments: { payload: { messages } } } } = res;
      refetchPayments();
      ...
      <Container style={styles.container}>
        ...
        >
          {({
            ...
            const { myPayment: { payload } } = data;
            const paymentStatus = StringUtils.paymentStatus(payload.status);

            const refetchListToDetails = () => {
              refetch();
              refetchPayments();
            };
            ...
            if (
              ...
              && !payload.attachments.includes('add image')
            ) {
              payload.attachments.push('add image');
            }
                ...
                <View style={styles.parallaxChild}>
                  ...
                  <Item style={styles.paymentHeader}>
                    <Left style={styles.paymentHeaderLeft}>
                      <UserAvatar size={40} fontDecrease="2" name={StringUtils.getInitials(payload.merchantName)} />
                    </Left>
                    <Body style={styles.paymentHeaderBody}>
                      <Text numberOfLines={1} ellipsizeMode="tail" style={styles.merchantName}>
                    ...
                    <Right style={styles.paymentHeaderRight}>
                      <Text numberOfLines={1} style={styles.amountTotal}>
                        {NumberUtils.formatCurrency(payload.currency, payload.amountTotal)}
                      </Text>
                    </Right>
                  </Item>
                  <TabSelection
