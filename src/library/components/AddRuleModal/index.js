...
const AddRuleModal = ({
  ...
  <Modal isVisible={isVisible}>
    <View style={styles.modal}>
      <Text style={styles.title}>{R.strings.addRule}</Text>
      <KeyboardAvoidingView enabled behavior="padding" style={styles.body}>
        <View style={styles.item}>
          <Label style={styles.label}>{R.strings.type}</Label>
          <Picker
            mode="dropdown"
            iosIcon={<Icon name="arrow-down" />}
            style={{ width: undefined }}
            selectedValue={addRuleType}
            onValueChange={handleAddRuleType}
            placeholderIconColor={R.colors.subhead}
          >
            {
              !isEmpty(rules) && rules.map(rule => (
                <Picker.Item key={rule.id} label={rule.text} value={rule.id} />
              ))
            }
          </Picker>
        </View>
...
AddRuleModal.propTypes = {
  addRuleType: PropTypes.number,
  isVisible: PropTypes.bool,
  onCancel: PropTypes.func,
  slider: PropTypes.number,
  handleSlider: PropTypes.func,
  handleAddRuleType: PropTypes.func,
